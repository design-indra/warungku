        ...
        <div className="flex min-w-max">
          {[
            ...
            <button
              key={k}
              onClick={() => setTab(k)}
              className={`px-4 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${tab === k ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400'}`}
            >
              {l}
            </button>
          ))}
              ...
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {t.pelanggan?.nama && (
                      <p className="text-xs text-blue-600 mt-0.5">👤 {t.pelanggan.nama}</p>
                    )}
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
                  ...
                  {/* Tombol Batalkan — hanya untuk transaksi yang belum batal */}
                  {t.status !== 'batal' && (
                    <button
                      onClick={() => handleCancel(t)}
                      disabled={cancelling === t.id}
                      className="flex items-center gap-1 text-xs text-red-500 font-semibold hover:text-red-700 transition-colors disabled:opacity-50"
                    >
                      <XCircle className="w-3.5 h-3.5" />
                      {cancelling === t.id ? 'Membatalkan...' : 'Batalkan'}
                    </button>
                  )}
                </div>
              </div>
                  ...
                  {t.detail_transaksi.map((d, i) => (
                    <div key={i} className="flex justify-between text-xs">
                      ...
                      <span className="text-gray-500">
                        {d.qty} × {rp(d.harga_jual)} = <span className="font-semibold text-gray-800">{rp(d.subtotal)}</span>
                      </span>
                    </div>
                  ))}
                  {t.diskon > 0 && (
                    <div className="flex justify-between text-xs pt-1 border-t border-gray-200">
                      ...
                      <span className="text-red-500 font-semibold">-{rp(t.diskon)}</span>
                    </div>
                  )}
                </div>
              )}
...
function PembayaranView({ total, diskon, onBack, onBayar, saving }) {
  ...
  // Fetch pelanggan saat metode hutang dipilih
  useEffect(() => {
    if (metode === 'hutang' && pelangganList.length === 0) {
      setLoadingPelanggan(true)
      fetch('/api/pelanggan')
        .then(r => r.json())
        .then(j => setPelangganList(j.data || []))
        .catch(() => {})
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div className="space-y-2">
            {METODE.map(m => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all ${metode === m.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${metode === m.id ? 'bg-blue-600' : 'bg-gray-100'}`}>
                  <m.icon className={`w-4 h-4 ${metode === m.id ? 'text-white' : 'text-gray-500'}`} />
                </div>
                <span className={`flex-1 text-left font-semibold text-sm ${metode === m.id ? 'text-blue-700' : 'text-gray-700'}`}>
                  {m.label}
                </span>
                {m.id === 'hutang' && (
                  <span className="text-[10px] text-yellow-600 font-bold bg-yellow-100 px-2 py-0.5 rounded-full">
                    Catat Piutang
                  </span>
                )}
                {metode === m.id && (
        ...
        {/* Pilih pelanggan — hanya untuk hutang */}
        {metode === 'hutang' && (
          <div>
            <p className="text-sm font-semibold text-gray-700 mb-2">Pilih Pelanggan <span className="text-red-500">*</span></p>
            {loadingPelanggan ? (
              <p className="text-xs text-gray-400">Memuat pelanggan...</p>
            ) : (
              <select
                value={pelangganId}
                onChange={e => setPelangganId(e.target.value)}
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-900 focus:outline-none focus:border-blue-400 bg-white"
              >
                <option value="">— Pilih pelanggan —</option>
                {pelangganList.map(p => (
                  <option key={p.id} value={p.id}>{p.nama}{p.no_hp ? ` (${p.no_hp})` : ''}</option>
                ))}
              </select>
            )}
            {!pelangganId && (
              <p className="text-xs text-red-500 mt-1">Pilih pelanggan untuk mencatat hutang.</p>
            )}
          </div>
        )}
      ...
      <div className="p-4 border-t border-gray-100 flex-shrink-0">
        <button
          onClick={() => onBayar({
            ...
            pelanggan_id: metode === 'hutang' ? pelangganId : null,
          })}
        ...
        >
          ...
          {saving ? 'Memproses...' : metode === 'hutang' ? 'CATAT HUTANG' : 'BAYAR'}
        </button>
      </div>
    </div>
  )
...
// ─── KERANJANG PANEL ──────────────────────────────────────────────────────────
function KeranjangPanel({
  cart, subtotal, totalItem, diskon, setDiskon, diskonNominal,
  total, updateQty, removeFromCart, clearCart, onBayar, onClose
}) {
  return (
    <>
...
export default function KasirPage() {
  ...
  // Ref untuk focus ke search saat scan barcode
  ...
  // ── Scan barcode: fokus ke search field ──
  ...
  // Saat scan barcode menghasilkan kode, langsung tambah ke keranjang jika tepat 1 produk
  const handleSearchChange = (val) => {
    setSearch(val)
  }

  // Jika hasil pencarian tepat 1, auto-add ke keranjang (logika barcode)
  useEffect(() => {
    if (!search) return
  ...
  const updateQty = (id, d) => setCart(prev => prev.map(c =>
    c.id === id ? { ...c, qty: Math.max(1, Math.min(c.qty + d, products.find(p => p.id === id)?.stok || 999)) } : c
  ))
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-gray-50">
      ...
      <div className="bg-white px-4 py-2.5 border-b border-gray-100 flex gap-2 flex-shrink-0">
        ...
        {/* Tombol Scan Barcode — fokus ke search, lalu scan eksternal isi kode */}
        <button
          onClick={handleScanClick}
          title="Scan barcode (fokus ke kolom pencarian)"
        ...
        <div className="flex gap-2 overflow-x-auto pt-2.5 hide-scrollbar">
          {categories.map(c => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold border transition-colors flex-shrink-0 ${activeCategory === c ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'}`}
            >
              {c}
            </button>
          ))}
                  ...
                  {p.foto_url ? (
                    <img
                      ...
                      onError={e => {
                        ...
                        e.target.parentElement.innerHTML = p.emoji || '📦'
                      }}
                    />
                  ) : (
                    p.emoji || '📦'
      ...
      <div className="bg-white border-t border-gray-200 px-4 py-3 flex-shrink-0">
        ...
        <button
          ...
          className={`w-full py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-colors ${cart.length > 0 ? 'bg-blue-700 hover:bg-blue-800 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
        >
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm max-w-sm mx-auto">
          <div className="text-center mb-5">
            ...
            <p className="font-extrabold text-lg text-gray-900">
              {tx.metode_bayar === 'hutang' ? 'Hutang Dicatat' : 'Pembayaran Berhasil'}
            </p>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span>{tx.metode_bayar === 'hutang' ? <span className="text-yellow-600 font-semibold">Hutang</span> : rp(tx.bayar)}</span>
            </div>
            {tx.kembalian > 0 && (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {btStatus !== 'connected' ? (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 py-4 border-2 rounded-2xl font-semibold text-sm transition-colors ${btStatus === 'connecting' ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100'}`}
              >
                {btStatus === 'connecting' ? (
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 py-4 border-2 rounded-2xl font-semibold text-sm transition-colors ${btStatus === 'printing' ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-700 border-blue-700 text-white hover:bg-blue-800'}`}
              >
                {btStatus === 'printing' ? (
            ...
            <div className="flex gap-1">
              {[
                ...
                <button
                  ...
                  className={`px-2 py-1 text-[10px] font-bold rounded-lg border transition-colors ${paperWidth === w ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 text-gray-500 border-gray-200'}`}
                >
                  {l}
                </button>
              ))}
