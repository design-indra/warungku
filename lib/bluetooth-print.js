...
async function connectCapacitor() {
  ...
  // Ambil daftar device yang sudah pernah dipasangkan (paired)
  const { devices } = await BluetoothSerial.list()

  if (!devices || devices.length === 0) {
    throw new Error(
      'Tidak ada printer Bluetooth yang tersambung.\n' +
      'Buka Pengaturan → Bluetooth → Pasangkan printer terlebih dahulu, lalu coba lagi.'
    )
  }

  // Filter device yang kemungkinan printer (nama mengandung kata printer umum)
  const printerKeywords = ['print', 'xp', 'pt', 'rp', 'tm', 'pos', 'thermal', 'bt', 'mtp', 'mpp']
  let target = devices.find(d =>
    printerKeywords.some(k => d.name?.toLowerCase().includes(k))
  ) || devices[0]  // fallback: device pertama
...
export async function printStruk(tx, store, paperWidth = 32) {
  ...
  if (isCapacitorApp()) {
    ...
    await BluetoothSerial.write({ data: b64 })
  } else {
